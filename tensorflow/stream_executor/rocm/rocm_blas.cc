...
static bool fold(memory_copy_op& y, const memory_copy_op& x) {
  ...
  if (x.src_ptr == y.src_ptr && x.size == y.size &&
      ...
      !misaligned && !(dst_step & 3)) {
    if (y.count == 1) y.dst_stride = dst_step;
    y.count++;
    return true;
    ...
    bool gather)
{
  ...
  for (int i = 1; i < batch_count; ++i) {
    ...
    if (fold(op, x)) continue;
    if (ops.size() > 1 && fold(ops[ops.size() - 2], op)) ops.pop_back();
    ops.push_back(x);
  }
  if (ops.size() > 1 && fold(ops[ops.size() - 2], ops.back())) ops.pop_back();
  for (auto& x : ops) {
    if (x.count > 1) {
      broadcast_fp32(AsGpuStreamValue(stream),
                     reinterpret_cast<float*>(x.dst_ptr), x.dst_stride >> 2,
                     x.count, reinterpret_cast<float*>(x.src_ptr), x.size >> 2);
    } else {
