...
def load_model_from_hdf5(filepath, custom_objects=None, compile=True):  # pylint: disable=redefined-builtin
  ...
  try:
    ...
    model_config = json.loads(try_decode(model_config,'utf-8'))
    ...
    if compile:
      ...
      if training_config is None:
        ...
        return model
      training_config = json.loads(try_decode(training_config,'utf-8'))
...
def load_weights_from_hdf5_group(f, layers):
  ...
  if 'keras_version' in f.attrs:
    original_keras_version = try_decode(f.attrs['keras_version'],'utf8')
  else:
    original_keras_version = '1'
  if 'backend' in f.attrs:
    original_backend = try_decode(f.attrs['backend'],'utf8')
  else:
    original_backend = None
...
def load_attributes_from_hdf5_group(group, name):
  ...
  if name in group.attrs:
    data = [try_decode(n,'utf8') for n in group.attrs[name]]
  else:
    data = []
    chunk_id = 0
    while '%s%d' % (name, chunk_id) in group.attrs:
      data.extend(
          [try_decode(n,'utf8') for n in group.attrs['%s%d' % (name, chunk_id)]])
      chunk_id += 1
  return data
