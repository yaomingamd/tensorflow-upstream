...
//  uint32_t nextbit = 1<<(mfmt-wm-1);
  int drop_mask_len = mfmt-wm;
  uint32_t drop_mask =  (1 << (mfmt-wm)) - 1;

  int new_exponent = 0, new_mantissa = 0;
  //const int max_exp = (1<<we)-(negative_zero_nan ? 1 : 2);
  bool midpoint = ((mantissa & drop_mask)!=0)
    && ((mantissa & ((1<<(drop_mask_len-1))-1))==0);
  bool first_significant_bit = (mantissa >> drop_mask_len) & 1;

  uint32_t delta = (stoch ? rng : mantissa) & drop_mask;
  if(midpoint && !first_significant_bit && !stoch)
    delta--;
  mantissa += delta;
  if(exponent!=0)
    mantissa += 1<<mfmt;
...
uint8_t cast_to_f8(T _x, bool stoch, uint32_t rng) {
  ...
  uint32_t drop_mask_len = mfmt-wm;
  const int max_exp = (1<<we)-(negative_zero_nan ? 1 : 2);
  const int exp_low_cutoff = (sizeof(T)==4 ? 128 : 16) - (1<<(we-1)) + 1 - (negative_zero_nan ? 1 : 0);

  exponent -= exp_low_cutoff-1;
  if(exponent<=0) 
    drop_mask_len = mfmt-wm+1-exponent;
  uint32_t drop_mask =  (1 << drop_mask_len) - 1;
  mantissa += 1<<mfmt;
