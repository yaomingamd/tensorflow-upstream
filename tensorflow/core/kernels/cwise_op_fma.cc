...
template <FMAType Type, typename T> T fma_op(T m1, T m2)
{
    if (Type==FMAType_Add)
      return m1 + m2;
...
void LaunchFusedMulAddOp<Device,T,Type>::operator()(const Device& device,
		T* out,
...
template <typename Device, typename T, FMAType Type>
void LaunchFusedMulAdd2Op<Device,T,Type>::operator()(const Device& device,
	T* out,
...
//    int64 out_dims[6]={1,1,1,1,1,1};
    ...
    if(pure_broadcast)
      ...
      FallbackLaunchFusedMulAddOp<Device,T,Type>()(ctx->eigen_device<Device>(), 
        output->flat<T>().data(), 
        inputs[0]->flat<T>().data(), inputs[1]->flat<T>().data(), 
        inputs[2]->flat<T>().data(), 
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if(pure_broadcast)
      ...
      FallbackLaunchFusedMulAdd2Op<Device,T,Type>()(ctx->eigen_device<Device>(), 
        output->flat<T>().data(), 
        inputs[0]->flat<T>().data(), inputs[1]->flat<T>().data(), 
        inputs[2]->flat<T>().data(), inputs[3]->flat<T>().data(),
        &out_dims[0], &broadcast_masks[0]);
