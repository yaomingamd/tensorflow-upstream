...
void LaunchFusedMulAddOp<Device, T, SGN>::operator()(
...
template <typename Device, typename T, int SGN>
void LaunchFusedMulAdd2Op<Device, T, SGN>::operator()(
    const Device& device, T* out, const T* x1, const T* y1, const T* x2,
...
template <typename Device, typename T, int SGN>
void FallbackLaunchFusedMulAddOp<Device, T, SGN>::operator()(
    const Device& device, T* out, const T* x1, const T* y1, const T* x2,
...
template <typename Device, typename T, int SGN>
void FallbackLaunchFusedMulAdd2Op<Device, T, SGN>::operator()(
    const Device& device, T* out, const T* x1, const T* y1, const T* x2,
...
class FusedMulAddBase {
 public:
  ...
  bool DoShapeAnalysis(OpKernelContext* ctx, const Tensor** inputs,
                       bool& pure_broadcast, uint8* broadcast_masks,
                       int64* out_dims, TensorShape& out_shape,
                       int64& out_elements) {
    int64 in_elements[N];
    int in_ranks[N];
    int rank = 0;
    ...
    for (int i = 0; i < N; i++) {
      ...
      /*
         ...
         in_elements[i]); for(int j=0; j<in_ranks[i]; j++) printf("%d ",
         inputs[i]->shape().dim_size(j)); printf("\n");
     */
    }
    ...
    */
    ...
    for (int i = 0; i < rank; i++) {
      ...
      for (int j = 0; j < N; j++) {
        if (ii < in_ranks[j])
          ...
          xds[i][j] = 1;
        if (xds[i][j] == 0) xds[i][j] = 1;
        max_dim = (max_dim > xds[i][j]) ? max_dim : xds[i][j];
      }
      for (int j = 0; j < N; j++) {
        if (!(xds[i][j] == 1 || xds[i][j] == max_dim)) return false;
    ...
    pure_broadcast?"true":"false"); for(int i=0; i<5; i++) printf("out_dim[%d] =
...
class FusedMulAddOp : public OpKernel, public FusedMulAddBase<Device, 3> {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (pure_broadcast)
      LaunchFusedMulAddOp<Device, T, SGN>()(
      ...
      FallbackLaunchFusedMulAddOp<Device, T, SGN>()(
          ctx->eigen_device<Device>(), output->flat<T>().data(),
          inputs[0]->flat<T>().data(), inputs[1]->flat<T>().data(),
          inputs[2]->flat<T>().data(), out_dims, broadcast_masks);
  }
};
...
class FusedMulAdd2Op : public OpKernel, public FusedMulAddBase<Device, 4> {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, out_shape, &output));
    if (pure_broadcast)
      LaunchFusedMulAdd2Op<Device, T, SGN>()(
          ctx->eigen_device<Device>(), output->flat<T>().data(),
          inputs[0]->flat<T>().data(), inputs[1]->flat<T>().data(),
      ...
      FallbackLaunchFusedMulAdd2Op<Device, T, SGN>()(
          ctx->eigen_device<Device>(), output->flat<T>().data(),
          inputs[0]->flat<T>().data(), inputs[1]->flat<T>().data(),
          inputs[2]->flat<T>().data(), inputs[3]->flat<T>().data(), out_dims,
          broadcast_masks);
  }
};
