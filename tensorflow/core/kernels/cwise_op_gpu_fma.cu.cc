...
template <int N>
void LaunchFusedMulAddOp<GPUDevice, T, SGN>::execute(const GPUDevice& device,
                                                     T* out, const T* x1,
                                                     const T* y1, const T* x2,
                                                     uint64 elements) {
  // printf("LaunchFusedMulAddOp<GPU,%d>(%ld)\n", N, elements);
  auto config = GetGpuLaunchConfig(elements, device);
  TF_CHECK_OK(GpuLaunchKernel(CwiseFusedMulAddKernel<T, N, SGN>,
                              config.block_count, config.thread_per_block, 0,
                              device.stream(), config, out, x1, y1, x2));
}

template <typename T, int SGN>
void LaunchFusedMulAddOp<GPUDevice, T, SGN>::operator()(
...
void LaunchFusedMulAdd2Op<GPUDevice, T, SGN>::execute(const GPUDevice& device,
  ...
  auto config = GetGpuLaunchConfig(elements, device);
  TF_CHECK_OK(GpuLaunchKernel(CwiseFusedMulAdd2Kernel<T, N, SGN>,
                              config.block_count, config.thread_per_block, 0,
                              device.stream(), config, out, x1, y1, x2, y2));
}

template <typename T, int SGN>
void LaunchFusedMulAdd2Op<GPUDevice, T, SGN>::operator()(
    const GPUDevice& device, T* out, const T* x1, const T* y1, const T* x2,
...
template <typename T, int SGN>
void FallbackLaunchFusedMulAddOp<GPUDevice, T, SGN>::operator()(
    const GPUDevice& device, T* out, const T* x1, const T* y1, const T* x2,
  ...
  if (dims[2] == 1 && dims[3] == 1 && dims[4] == 1) {
    ...
    TF_CHECK_OK(GpuLaunchKernel(FallbackLaunchFusedMulAddKernel<T, SGN>,
                                dim3(grid_x, grid_y, grid_z),
                                dim3(block_x, block_y, block_z), 0,
                                device.stream(), out, x1, y1, x2, arg));
  }
...
template <typename T, int SGN>
void FallbackLaunchFusedMulAdd2Op<GPUDevice, T, SGN>::operator()(
    const GPUDevice& device, T* out, const T* x1, const T* y1, const T* x2,
  ...
  TF_CHECK_OK(GpuLaunchKernel(FallbackLaunchFusedMulAdd2Kernel<T, SGN>,
                              dim3(grid_x, grid_y, grid_z),
                              dim3(block_x, block_y, block_z), 0,
