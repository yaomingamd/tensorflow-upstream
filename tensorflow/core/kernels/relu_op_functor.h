...
*/    
    ...
    activations.device(d) = features.constant(half) * features 
      * (features.constant(one) + (features.constant(p1)*features + features.constant(p3) * features*features*features).tanh());
  }
};
