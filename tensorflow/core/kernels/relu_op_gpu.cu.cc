...
template <class T>
__global__ void GeluGradKernel(const T* gradient,
                                 const T* feature,
                                 T* backprop, int32 count) {
  int i = threadIdx.x + blockIdx.x*blockDim.x;
  if(i>=count)
    return;
  ...
  T g = gradient[i];
  T cz = 1./cosh(z);
  backprop[i] = static_cast<T>(g * 0.5*(1. + tanh(z) + x*(p1+3*p3*x*x)*cz*cz)); 
}
...
__global__ void GeluGradKernel<Eigen::half>(const Eigen::half* _gradient,
                                 ...
                                 Eigen::half* _backprop, int32 count) {
  int i = threadIdx.x + blockIdx.x*blockDim.x;
  if(i>=count)
    return;  
  const float scale = 0.7978845608028654;
...
struct Gelu<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T>::ConstTensor input,
    ...
    TF_CHECK_OK(GpuLaunchKernel(
        GeluKernel<T>, (count+kThreadInBlock-1)/kThreadInBlock, kThreadInBlock, 0,
        d.stream(), input.data(), output.data(), count));
  }
};
...
struct GeluGrad<GPUDevice, T> {
  void operator()(const GPUDevice& d,
                  typename TTypes<T>::ConstTensor gradient,
    ...
    constexpr int32 kThreadInBlock = 256;
    TF_CHECK_OK(GpuLaunchKernel(
        GeluGradKernel<T>, (count+kThreadInBlock-1)/kThreadInBlock, kThreadInBlock, 0,
        d.stream(), gradient.data(), feature.data(), backprop.data(), count));
  }
};
#endif
