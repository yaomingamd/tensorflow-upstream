...
__device__ void apply_dropout(T& out, half2 rng,
                              half2 rate, half2 scale) {
  half2 mask = make_half2(__low2half(rng)>__low2half(rate), __high2half(rng)>__high2half(rate));
  out = __hmul2(__hmul2(mask,scale),out);
}

template <class T, class U>
__device__ void apply_dropout(T& out, half2 rng,
                              half2 rate, float2 scale) {
  out.x *= scale.x;
  out.y *= scale.y;
...
template<>
__device__ void apply_dropout<half2, half>(half2& out, half2 rng,
                              half2 rate, float2 scale) {
  half2 mask = make_half2(__low2half(rng)>__low2half(rate), __high2half(rng)>__high2half(rate));
...
__global__ void RNGAndApplyDropoutKernel(random::PhiloxRandom gen, uint32 size,
  ...
  typedef
      typename std::conditional<is_half || is_half2, half2,
...
void ApplyDropout<GPUDevice, T>::operator()(const GPUDevice& d, T* out,
  ...
  if(std::is_same<T, float>::value && num_blocks>256)
    num_blocks = 256;

  if (do_half2) {
    TF_CHECK_OK(GpuLaunchKernel(
        RNGAndApplyDropoutKernel<half2, half2, half2>, num_blocks, kThreadInBlock,
        0, d.stream(), gen, num_elements, reinterpret_cast<half2*>(out),
        reinterpret_cast<const half2*>(in), __floats2half2_rn(rate, rate),
        __floats2half2_rn(scale, scale)));
  } else {
    TF_CHECK_OK(GpuLaunchKernel(RNGAndApplyDropoutKernel<T, half2, float2>, num_blocks,
                                kThreadInBlock, 0, d.stream(), gen,
                                num_elements, out, in, __floats2half2_rn(rate, rate),
