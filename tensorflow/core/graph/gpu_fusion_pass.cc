...
inline bool isOpAddX(const Node* n) { return (n->type_string() == "Add") 
  || (n->type_string() == "AddV2"); }
...
public:
  ...
  void InitializeFusions(
      std::vector<std::unique_ptr<ROCmFusionOpBase> >& fusions, Graph* g) override;
};
...
Status ROCmFMAPass::Run(const GraphOptimizationPassOptions& options) {
  if (ReadBoolFromEnvVar("TF_ROCM_FMA_DISABLE")) 
    return Status::OK();
...
bool ROCmFusionOpFMA::IsFusionEligible(const Node* node, FusionOpData* d) {
  ...
  VLOG(kVlogLevel)<<"Trying to fuse "<<node->type_string()<<" "<<node->in_edges().size()
    <<b->type_string()<<" "<<b->in_edges().size()<<" "<<b->out_edges().size()<<" "
    <<c->type_string()<<" "<<c->in_edges().size()<<" "<<c->out_edges().size();
